...
import com.epolixa.bityard.item.ItemGuardianEye;
...
public class RenderGuardianEyeBeam extends RenderPlayer
{
    private static final ResourceLocation GUARDIAN_BEAM_TEXTURE = new ResourceLocation("textures/entity/guardian_beam.png");

    public RenderGuardianEyeBeam(RenderManager renderManagerIn)
    {
    ...
    {
        Item heldItem = entity.getHeldItem(entity.getActiveHand()).getItem();

        if (heldItem instanceof ItemGuardianEye)
        {
            ItemGuardianEye guardianEye = (ItemGuardianEye)heldItem;
            EntityLiving targetedEntity = guardianEye.getTargetedEntity();

            if (targetedEntity != null)
            {
                float f = guardianEye.getAttackAnimationScale(partialTicks);
                ...
                Vec3d vec3d = this.getPosition(targetedEntity, (double)targetedEntity.height * 0.5D, partialTicks);
